...
import app.assessmentdubizzle.com.di.listener.ItemClickListener
...
class ListFragment : Fragment(), ItemClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        dynamoVM.getData().observe(viewLifecycleOwner, {
            if (it.results.isNullOrEmpty()){
                ...
                listAdapter.setDynamoWriterListData(it.results as ArrayList<ResultsItem>?)
            }
        })
    ...
    private fun initAdapter() {
        ...
        listAdapter = ListAdapter(this, _binding?.root?.context)
        _binding?.rvDynamoWriter?.adapter = listAdapter
    }
    ...
    override fun onAdapterClickListener(position: Int, view: View, resultItem: ResultsItem?) {
        Navigation.findNavController(view).navigate(ListFragmentDirections.actionDynamoWriterListFragmentToDynamoWriterDetailsFragment(resultItem))
    }
